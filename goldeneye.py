...
# === Modifikasi Pengulangan Otomatis Ditambahkan ===
...
# Konfigurasi
URL = sys.argv[1]
WORKERS = sys.argv[sys.argv.index("-w") + 1] if "-w" in sys.argv else "10"
SOCKETS = sys.argv[sys.argv.index("-s") + 1] if "-s" in sys.argv else "500"
MAX_FAILED = 100  # Batas maksimal failed untuk mengulang
RETRY_INTERVAL = 5  # Jeda waktu sebelum mengulang (detik)
    ...
    while True:
        ...
        try:
            ...
            while True:
                ...
                # Menghitung jumlah "Failed" dari output
                match = re.search(r"(\d+) Failed", output)
                if match:
                    failed_count = int(match.group(1))
                    print(f"Jumlah koneksi gagal saat ini: {failed_count}")

                # Jika jumlah failed melebihi batas, restart
                if failed_count >= MAX_FAILED:
                    print("Jumlah failed melebihi batas. Mengulangi kembali...")
                    process.kill()
            ...
            print(f"Error: {e}. Mengulangi dalam {RETRY_INTERVAL} detik.")
            time.sleep(RETRY_INTERVAL)
